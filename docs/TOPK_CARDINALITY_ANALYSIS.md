...
- [x] Count-Based 方法测试 - **不适用**
  - 虽然减少比较次数，但增加内存写入
  - 10M 数据量下稳定 ~3ms，比 DuckDB ~1.7ms 慢

- [x] SIMD 线性搜索测试 - **不适用**
  - O(n * cardinality) 复杂度太高
...
| 基数 | DuckDB (ms) | ThunderDuck (ms) | 加速比 |
|------|-------------|------------------|--------|
| 10 | 1.66 | 14.72 | 0.11x |
| 50 | 1.78 | 5.23 | 0.34x |
| 200 | 1.79 | 2.05 | 0.87x |
| **500** | 1.74 | 1.19 | **1.46x** |
| 1000 | 2.05 | 0.82 | 2.51x |
| 5000 | 2.15 | 0.64 | 3.39x |
| 10000 | 2.34 | 0.64 | 3.67x |
| 100000 | 2.63 | 0.62 | 4.26x |
| 10M | 2.68 | 0.66 | 4.03x |
