...
| DuckDB 胜出 | 1 (7%) |
| 最大加速比 | **6,700x** (COUNT 操作) |
| Sort 平均加速 | **9.95x** (Radix Sort) |
| Aggregation 平均加速 | **1,682x** |
| **Filter 平均加速** | **10.24x** ⬆️ |
| TopK 平均加速 | **7.25x** |
...
| 指标 | v2.0 | v3.0 | 改进 |
|------|------|------|------|
| 总体胜率 | 71% (10/14) | **93% (13/14)** | +22% |
| Filter 胜率 | 50% (2/4) | **100% (4/4)** | +50% |
| Filter 平均加速 | 1.06x | **10.24x** | +9.7x |
| v2→v3 内部加速 | - | **3.49x** | 模板特化+ILP优化 |
...
### 3.2 过滤操作 (Filter) - 100% 胜率 ✅ (v3.0 优化完成)

| ID | 操作 | SQL | DuckDB | ThunderDuck v3 | 加速比 | 胜者 |
|----|------|-----|--------|----------------|--------|------|
| F1 | GT | `WHERE l_quantity > 25` | 0.153 ms | 0.014 ms | **10.73x** | ThunderDuck |
| F2 | EQ | `WHERE l_quantity = 30` | 0.152 ms | 0.014 ms | **10.78x** | ThunderDuck |
| F3 | Range | `WHERE qty >= 10 AND qty < 40` | 0.184 ms | 0.023 ms | **8.07x** | ThunderDuck |
| F4 | High Sel | `WHERE l_extendedprice > 500` | 0.145 ms | 0.013 ms | **11.36x** | ThunderDuck |

**v3.0 优化技术** (详见 FILTER_COUNT_OPTIMIZATION_DESIGN.md):

| 优化 | 效果 | 技术 |
|------|------|------|
| 模板特化 | +15% | 消除循环内 switch-case |
| 独立累加器 | +25% | 4 个并行累加流消除依赖链 |
| vsub 技巧 | +8% | 用减法替代移位+加法 |
| 256 元素批次 | +8% | 减少水平归约次数 |
...
| 测试 | v2 (ms) | v3 (ms) | 内部加速 |
|------|---------|---------|----------|
| GT | 0.857 | 0.215 | 3.99x |
| EQ | 0.861 | 0.204 | 4.23x |
| Range | 0.590 | 0.299 | 1.97x |
