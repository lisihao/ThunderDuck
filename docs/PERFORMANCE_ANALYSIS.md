...
| DuckDB 胜出 | 1 (7%) |
| 最大加速比 | **46,000x** (COUNT 操作) |
| Sort 平均加速 | **5.0x** (Radix Sort) |
| Aggregation 平均加速 | **11,500x** |
| **Filter 平均加速** | **3.95x** ⬆️ |
| **TopK 平均加速** | **13.2x** ⬆️ |
...
| Filter 平均加速 | 1.06x | **3.95x** | +2.9x |
| TopK 胜率 | 66% (2/3) | **100% (3/3)** | +34% |
| TopK 平均加速 | 1.96x | **13.2x** | +11.2x |
| Filter v2→v3 内部加速 | - | **3.49x** | 模板特化+ILP优化 |
| TopK v2→v3 内部加速 | - | **18.5x** | SIMD批量跳过+自适应策略 |
...
### 3.4 Top-K 操作 - 100% 胜率 ✅ (v3.0 优化完成)

| ID | 操作 | K 值 | DuckDB | ThunderDuck v3 | 加速比 | 胜者 |
|----|------|------|--------|----------------|--------|------|
| T1 | Top-10 | 10 | 1.058 ms | 0.457 ms | **2.31x** | ThunderDuck |
| T2 | Top-100 | 100 | 1.265 ms | 0.049 ms | **25.73x** | ThunderDuck |
| T3 | Top-1000 | 1000 | 1.979 ms | 0.171 ms | **11.57x** | ThunderDuck |

**v3.0 优化技术** (详见 TOPK_OPTIMIZATION_DESIGN.md):

| 优化 | 效果 | 技术 |
|------|------|------|
| SIMD 批量跳过 | +10-40x | vmaxvq_u32 快速检测候选批次 |
| 自适应 K 阈值 | +5x | K ≤ 1024 用 SIMD 堆，K > 1024 用 nth_element |
| 阈值缓存 | +15% | 避免重复读取堆顶 |
...
| 测试 | v2 (ms) | v3 (ms) | 内部加速 |
|------|---------|---------|----------|
| Top-100 | 0.50 | 0.05 | 10x |
| Top-1000 | 3.15 | 0.17 | 18.5x |
...
|------|--------|--------|------|-----------|-----------|
| Aggregation | 4 | 4 | **100%** | 11,502x | 45,996x |
| Sort | 2 | 2 | **100%** | 5.0x | 5.19x |
| TopK | 3 | 3 | **100%** ✅ | 13.2x | 25.73x |
| Filter | 4 | 4 | **100%** ✅ | 3.95x | 4.44x |
| Join | 1 | 0 | 0% | 0.07x | - |
