...
struct HashTable::Impl {
    ...
    size_t count_matches_i32(const int32_t* probe_keys, size_t probe_count) const {
        ...
        for (size_t i = 0; i < probe_count; ++i) {
            // 预取下一批哈希表槽位
            if (i + 8 < probe_count) {
                uint32_t prefetch_hash = hash_one_i32(probe_keys[i + 8]);
                uint32_t prefetch_bucket = prefetch_hash & (num_buckets - 1);
                __builtin_prefetch(&buckets[prefetch_bucket], 0, 0);
            }
    ...
    size_t count_matches_i64(const int64_t* probe_keys, size_t probe_count) const {
        ...
        for (size_t i = 0; i < probe_count; ++i) {
            if (i + 8 < probe_count) {
                uint64_t prefetch_hash = hash_one_i64(probe_keys[i + 8]);
                uint32_t prefetch_bucket = static_cast<uint32_t>(prefetch_hash & (num_buckets - 1));
                __builtin_prefetch(&buckets[prefetch_bucket], 0, 0);
            }
...
// v5: 计数方法 (两阶段算法)
size_t HashTable::count_matches_i32(const int32_t* probe_keys, size_t probe_count) const {
    return impl_->count_matches_i32(probe_keys, probe_count);
}

size_t HashTable::count_matches_i64(const int64_t* probe_keys, size_t probe_count) const {
    return impl_->count_matches_i64(probe_keys, probe_count);
}
...
void ensure_join_result_capacity(JoinResult* result, size_t exact_capacity) {
    ...
    // 分配精确容量 (不需要 2x 增长)
    result->left_indices = static_cast<uint32_t*>(
        aligned_alloc(exact_capacity * sizeof(uint32_t), CACHE_LINE_SIZE));
    result->right_indices = static_cast<uint32_t*>(
        aligned_alloc(exact_capacity * sizeof(uint32_t), CACHE_LINE_SIZE));
    result->capacity = exact_capacity;
    result->count = 0;
}
 ...
 *
 * ⚠️ 实验结果 (2026-01-26):
 * 测试场景 100K build × 1M probe 全匹配:
 * - V3: 0.92 ms
 * - V5: 3.44 ms (3.7x 更慢)
 *
 * 原因分析:
 * - 现代硬件 memcpy 极快 (20+ GB/s with SIMD)
 * - 额外哈希表遍历开销 >> grow_join_result 开销
 *
 * 结论: 保留作为参考实现，实际使用推荐 V3
 */
size_t hash_join_i32_v5(const int32_t* build_keys, size_t build_count,
    ...
    // Phase 1: 计数遍历 (不写入结果)
    size_t match_count = ht.count_matches_i32(probe_keys, probe_count);
    ...
    // 一次性精确分配 (无需 grow)
    ensure_join_result_capacity(result, match_count);

    // Phase 2: 填充结果
    size_t actual_count = ht.probe_i32(probe_keys, probe_count,
                                        result->left_indices, result->right_indices);
...
size_t hash_join_i64_v5(const int64_t* build_keys, size_t build_count,
    ...
    // Phase 1: 计数
    size_t match_count = ht.count_matches_i64(probe_keys, probe_count);
    ...
    // Phase 2: 填充
    size_t actual_count = ht.probe_i64(probe_keys, probe_count,
                                        result->left_indices, result->right_indices);
